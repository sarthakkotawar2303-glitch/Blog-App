  ...
  useEffect(() => {
    const fetchSavedPosts = async () => {
      ...
      try {
        ...
        if (res.isSuccess) setSavedPosts(res.data.savedPosts || []);
        else setError("Failed to fetch saved posts");
      } catch (err) {
      ...
      <div className="min-h-screen bg-[#0b0f1a]/85 py-12 px-6">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-6">
            <div>
              <h1 className="text-5xl font-black text-white tracking-tighter">Saved Posts</h1>
              <div className="h-1.5 w-16 bg-blue-600 rounded-full mt-3 mb-2"></div>
              <p className="text-slate-400 font-medium">Articles you've bookmarked to read later</p>
            </div>
          </div>
          ...
          {!loading && !error && savedPosts.length === 0 && (
            <div className="text-center py-32 bg-slate-900/40 rounded-[3rem] border border-dashed border-slate-800">
              <p className="text-slate-500 text-lg">You haven't saved any posts yet.</p>
            </div>
          )}
                ...
                <div className="p-8 flex flex-col flex-grow">
                  ...
                  <div className="flex items-center justify-between mt-8 pt-6 border-t border-white/5">
                    ...
                    <button
                      ...
                      className="p-3 text-blue-400 hover:text-white hover:bg-blue-500/20 rounded-full transition-all"
                      title="Remove Bookmark"
                    >
                      <FiBookmark className="fill-current" size={18} />
                    </button>
                  </div>
                </div>
              </article>
