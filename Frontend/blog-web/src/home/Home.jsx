...
const Home = () => {
  ...
  return (
    <div 
      className="min-h-screen bg-[#0b0f1a] text-slate-300 font-sans bg-fixed bg-cover bg-center"
      style={{ backgroundImage: `url(${backgroundImage})` }}
    >
     
      <div className="min-h-screen bg-[#0b0f1a]/80 backdrop-blur-[2px]">
        <div>
          <Banner />
        </div>
          ...
          <div className="flex flex-col lg:flex-row gap-12">
              ...
              <div className="sticky top-24 space-y-8">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-950/60 border border-slate-800 rounded-2xl py-3 pl-12 pr-4 
                               focus:outline-none focus:ring-2 focus:ring-blue-500/40 
                               transition-all text-sm text-white backdrop-blur-md"
                    onChange={(e) => setSearchQuery(e.target.value)}
                  />
                </div>

                <div className="bg-slate-950/60 border border-slate-800/60 rounded-3xl p-6 backdrop-blur-md shadow-xl">
            ...
            <main className="lg:w-3/4 w-full">
              <header className="flex justify-between items-center mb-12">
                ...
                <span className="text-slate-500 text-[10px] font-bold uppercase tracking-widest px-3 py-1 bg-slate-950/60 rounded-lg border border-slate-800 backdrop-blur-md">
              ...
              {loading ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {[1, 2, 3, 4].map((i) => (
                    <div key={i} className="h-80 bg-slate-900/50 animate-pulse rounded-[2.5rem]" />
                  ))}
                </div>
              ) : (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    {filteredPosts.map((post) => (
                      <div
                        key={post._id}
                        className="group flex flex-col bg-slate-950/40 border border-slate-800/60 rounded-[2.5rem] p-5 transition-all duration-300 hover:border-blue-500/30 hover:bg-slate-900/60 shadow-2xl backdrop-blur-sm"
                      >
                        <div className="relative aspect-[16/10] w-full overflow-hidden rounded-[1.8rem] mb-6 shadow-md">
                          ...
                          />
                          <div className="absolute inset-0 bg-gradient-to-t from-[#0b0f1a]/80 via-transparent to-transparent opacity-60" />
